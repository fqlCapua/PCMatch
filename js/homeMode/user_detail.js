var user_id = (parent.window.location.href).split("hid=")[1];
...
function load_introduce() {
    ...
    $.ajax({
            ...
            async: false,
            data: {
                service:"App.User.Guess_you_like",
                time: time_token()[0],
                token: time_token()[1],
                user_id: user_id,
            },
...
var userInfo = load_Info();
var Introduce=load_introduce();
...
var ImgStr=appendImg(userInfo.user_pics);
console.log(ImgStr);
$(".yulanImg").append($(ImgStr));
var app=angular.module("userInfoApp", []);
app.controller("userCtr", function($scope) {
    $scope.userInfo = userInfo;
     $scope.showArr = userInfo.user_pics;
     $scope.Introduce=Introduce;
    
    $scope.list = {
        "req_age": ["年龄", "18-23", "23-28", "28-33", "33-39", "39-43", "43-48", "48-53"],
        "user_Gender": ["性别", "男", "女"],
        "user_info_married": ["婚况", "未婚", "离异", "丧偶", "已婚"],
        "user_info_salary": ["收入", "<2000", "2000-3000", "3000-4000", "4000-5000", "5000-7000", "7000-1000", "10000-15000", "  15000-20000", " 20000-30000", ">30000"],
        "user_info_constellation": ["星座", "白羊座", "金牛座", "双子座", "巨蟹座", "狮子座", "处女座", "天秤座", "天蝎座", "射手座", "魔羯座", "水瓶座", "双鱼座"],
        "user_info_animal": ["生肖", "鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪"],
        "user_info_height": ["身高", "<140cm", "140-145cm", "145-150cm", "150-155cm", "155-160cm", "160-165cm", "165-170cm", "170-175cm", "175-180cm", "180-185cm", "185-190cm", "190-195cm",
            ">195cm"
        ],
        ...
        "user_info_child": ["子女情况", "没有", "有，和我住一起", "有，有时和我住一起", "有，不和我住一起"],
        "user_info_believe": ["信仰", "无神论", "佛教", "道教", "基督教", "天主教", "信神但未定教", "儒教", "犹太教", "回教", "伊斯兰教", "印度教", "其它"],
        "user_info_house": ["住房", "以后再告诉你", "与父母同住", "租房", "已购房(有贷款)", "已购房(无贷款)", "住单位房", "住亲朋家", "需要时购置"],
        "user_info_smoke": ["是否抽烟", "不吸，很反感", "不吸烟，但不反感", "社交时偶尔吸烟", "烟不离手"],
        "user_info_drink": ["是否喝酒", "不喝酒", "社交需要喝", "兴致时小酌", "酒不离口"],
        "user_info_car": ["是否购车", "未购车", "已购车", "单位用车", "需要时购置"],
        "user_info_cook": ["是否做饭", "色香味俱全", "能做几样可口的小菜", "不太会", "但愿为心爱的人学习"],
        "user_info_homework": ["是否家务", "任劳任怨", "希望对方承担家务", "一起分工合作", "看各自闲忙，协商分担"]
    };
